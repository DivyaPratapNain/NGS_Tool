...
class PipelineThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if self.mode == "diff":
                mode_folder = "Diff_Exp_Results/"
            elif self.mode == "lncrna":
                mode_folder = "LncRNA_Results/"
            else:
                mode_folder = "Variant_Calling_Results/"


            sam_dir = os.path.join(results_folder, mode_folder, "sam/")
            bam_dir = os.path.join(results_folder, mode_folder, "bam/")
            if self.mode == "diff":
                output_dir = os.path.join(results_folder, mode_folder, "counts/")
            elif self.mode == "lncrna":
                output_dir = os.path.join(results_folder, mode_folder, "lncrna_data/")
            else:
                output_dir = os.path.join(results_folder, mode_folder, "vcf/")

            os.makedirs(sam_dir, exist_ok=True)
            os.makedirs(bam_dir, exist_ok=True)
            os.makedirs(output_dir, exist_ok=True)

            fastq1_filename = os.path.basename(self.form_data["fastq1"])
            if "_" in fastq1_filename:
                fastq_prefix = fastq1_filename.split("_")[0]
            else:
                fastq_prefix = os.path.splitext(fastq1_filename)[0]
            self.log_signal.emit(f"🔍 Using FASTQ prefix: {fastq_prefix}")
            ...
            if self.mode == "diff":
                output_file = os.path.join(output_dir, f"{fastq_prefix}_gene_expression.txt")
            elif self.mode == "lncrna":
                output_file = os.path.join(output_dir, f"{fastq_prefix}_lncrna_expression.txt")
            else:
                output_file = os.path.join(output_dir, f"{fastq_prefix}_final_variants.vcf")

            def run_command(command, description):
                try:
                    self.log_signal.emit(f"🔄 Running: {description}...")
                    if self.stopped:
                        self.log_signal.emit("⛔️ Skipped command — process was stopped.")
                        return
                    
                    wsl_command = f'wsl bash -ic "{command}"'
            ...
            if self.mode == "diff":
                ...
                run_command(
                    f"featureCounts -T {self.form_data['threads']} -p -a {gtf_file_path} -o {output_path} "
                    f"-t {self.form_data['t_option']} -g {self.form_data['g_option']} {sorted_bam_path}",
                ...
                cpc2_local_path = os.path.join(base_dir, "cpc2", "bin", "CPC2.py")

                if not os.path.exists(cpc2_local_path):
                    self.log_signal.emit("❌ CPC2.py not found in bundled CPC2 directory!")
                    return
                self.log_signal.emit(f"📁 CPC2 absolute path: {os.path.abspath(cpc2_local_path)}")


                cpc2_abs_path = os.path.abspath(cpc2_local_path).replace("\\", "/")
                self.log_signal.emit(f"🧪 CPC2 full path before WSL: {cpc2_abs_path}")
                cpc2_script = self.convert_to_wsl_path(cpc2_abs_path)

                # Step 1: Generate STAR genome index if missing
                genome_index_file = os.path.join(ref_folder, "genomeParameters.txt")
                if not os.path.exists(genome_index_file):
                    self.log_signal.emit("🧠 STAR index not found! Creating STAR genome index now...")

                    annotation_gtf = self.convert_to_wsl_path(self.form_data["annotation"])
                    genome_fasta = self.convert_to_wsl_path(self.form_data["reference_genome"])


                    # Try-catch for STAR indexing
                    try:
                        threads = self.form_data["threads"]
                        ref_folder_wsl = self.convert_to_wsl_path(ref_folder)
                        genome_fasta = self.convert_to_wsl_path(self.form_data["reference_genome"])
                        annotation_gtf = self.convert_to_wsl_path(self.form_data["annotation"])

                        star_index_cmd = (
                            f'STAR --runThreadN {threads} --runMode genomeGenerate '
                            f'--genomeDir {ref_folder_wsl} --genomeFastaFiles {genome_fasta} '
                            f'--sjdbGTFfile {annotation_gtf} --sjdbOverhang 100'
                        )

                        self.log_signal.emit("🔧 Generating STAR index with:")
                        self.log_signal.emit(star_index_cmd)

                        success = run_command(star_index_cmd, "Generating STAR genome index")
                        if self.stopped: return
                        if not success:
                            raise RuntimeError("STAR index generation failed.")

                    except Exception as e:
                        self.log_signal.emit("❌ STAR genome index generation failed.")
                        self.log_signal.emit("⚠️ Reason: The annotation file might be incorrectly formatted.")
                        self.log_signal.emit("⚠️ Please ensure your GTF/GFF file contains 'exon' entries and matches the genome.")
                        self.log_signal.emit("⚠️ You can also manually generate the STAR index using the above command.")
                        self.completed_signal.emit()  # Let the UI reset properly
                        return
                ...
                genome_dir = self.convert_to_wsl_path(ref_folder)
                r1 = fastq1_path
                r2 = fastq2_path
                threads = self.form_data["threads"]

                # 1. Fix the output prefix (safe and portable)
                output_prefix = os.path.join(bam_dir, f"{fastq_prefix}_")
                output_prefix_wsl = self.convert_to_wsl_path(output_prefix)

                # 2. Handle gzip logic correctly
                read_command = "--readFilesCommand zcat" if fastq1_path.endswith(".gz") else ""

                # 3. STAR run with correct prefix and without broken temp dir
                run_command(
                    ...
                    f"--outFileNamePrefix {output_prefix_wsl}",
                    "Aligning with STAR"
                )
                ...
                annotated_gtf = os.path.join(output_dir, "compare.annotated.gtf")
                annotated_gtf_wsl = self.convert_to_wsl_path(annotated_gtf)

                run_command(
                    f"""bash -c "awk '$3==\\\"transcript\\\"' {annotated_gtf_wsl} | grep 'class_code \\"[{self.form_data['class_codes']}]\\"' > {novel_gtf_wsl}" """,
                    "Extracting novel transcript candidates"
                )
                

                if not os.path.exists(novel_gtf):
                        
                    self.log_signal.emit("⚠️ No novel transcript candidates found. The filtering step may have excluded all.")
                    return
                ...
                run_command(
                    f"featureCounts -T {self.form_data['threads']} -p "
                    f"-t {self.form_data['t_option']} -g {self.form_data['g_option']} "
                    f"-a {final_gtf_wsl} -o {self.convert_to_wsl_path(output_file)} {aligned_bam_wsl}",
    ...
    def stop(self):
        ...
        if self.running_process:
            try:
                self.running_process.kill()  # 💥 Instantly kill the process
                self.log_signal.emit("❌ Process forcefully killed.")
            except Exception as e:
                self.log_signal.emit(f"⚠️ Failed to kill process cleanly: {e}")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.start_button_style_orange = """
        QPushButton#start_button {
        ...
        if self.mode == "diff":
            ...
            self.annotation_file = self.create_input_with_browse(
                form_layout, "Annotation GTF/GFF:", 5, False,
                "GTF file from GENCODE or Ensembl used in gffcompare"
            )

            form_layout.addWidget(QLabel("Minimum lncRNA Length (bp):"), 6, 0)
            self.min_length = QLineEdit("200")
            form_layout.addWidget(self.min_length, 6, 1)

            form_layout.addWidget(QLabel("Class Codes to Include (e.g., uix):"), 7, 0)
            self.class_codes = QLineEdit("uix")
            form_layout.addWidget(self.class_codes, 7, 1)

            form_layout.addWidget(QLabel("Minimum FPKM (Expression):"), 8, 0)
            self.min_fpkm = QLineEdit("0.5")
            form_layout.addWidget(self.min_fpkm, 8, 1)

            t_lbl = QLabel("-t Option:")
            t_lbl.setToolTip("Feature type to count (e.g., exon, transcript)")
            form_layout.addWidget(t_lbl, 9, 0)
            self.t_option = QLineEdit("exon")
            form_layout.addWidget(self.t_option, 9, 1)

            g_lbl = QLabel("-g Option:")
            g_lbl.setToolTip("GTF attribute to group by (e.g., gene_id, transcript_id)")
            form_layout.addWidget(g_lbl, 10, 0)
            self.g_option = QLineEdit("gene_id")
            form_layout.addWidget(self.g_option, 10, 1)
        ...
        self.start_button.setStyleSheet(self.start_button_style_green)
    ...
    def toggle_pipeline(self):
        if self.pipeline_thread and self.pipeline_thread.isRunning():
            ...
            self.start_button.setStyleSheet(self.start_button_style_green)
            self.progress.hide()
            return  # ✅ THIS is what prevents it from running start_pipeline again
        else:
    ...
    def start_pipeline(self):
        ...
        if self.mode == "lncrna":
            ...
            "min_length": self.min_length.text(),
            "class_codes": self.class_codes.text(),
            "min_fpkm": self.min_fpkm.text(),
        ...
        self.start_button.setStyleSheet(self.start_button_style_orange)
